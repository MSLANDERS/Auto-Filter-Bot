...
async def next_page(bot, query):
    ...
    if settings['shortlink']:
        ...
        btn.insert(1,
            [InlineKeyboardButton("📥 𝗦𝗲𝗻𝗱 𝗔𝗹𝗹 𝗙𝗶𝗹𝗲𝘀 📤", url=await get_shortlink(settings['url'], settings['api'], f'https://t.me/{temp.U_NAME}?start=all_{query.message.chat.id}_{key}'))]
        )
    else:
        btn.insert(0,
            [InlineKeyboardButton("ʟᴀɴɢᴜᴀɢᴇs", callback_data=f"languages#{key}#{req}#{offset}"),
        ...
        btn.insert(1,
            [InlineKeyboardButton("📥 𝗦𝗲𝗻𝗱 𝗔𝗹𝗹 𝗙𝗶𝗹𝗲𝘀 📤", callback_data=f"send_all#{key}#{req}")]
        )
...
async def filter_languages_cb_handler(client: Client, query: CallbackQuery):
    ...
    if settings['shortlink']:
        btn.insert(1,
            [InlineKeyboardButton("🪄 sᴇɴᴅ ᴀʟʟ", url=await get_shortlink(settings['url'], settings['api'], f'https://t.me/{temp.U_NAME}?start=all_{query.message.chat.id}_{key}')),
            InlineKeyboardButton("ǫᴜᴀʟɪᴛʏ", callback_data=f"quality#{key}#{req}#{offset}")]
        )
    else:
        btn.insert(1,
            [InlineKeyboardButton("🪄 sᴇɴᴅ ᴀʟʟ", callback_data=f"send_all#{key}#{req}"),
            InlineKeyboardButton("ǫᴜᴀʟɪᴛʏ", callback_data=f"quality#{key}#{req}#{offset}")]
        )
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            ...
            InlineKeyboardButton('⚜️ Sᴜᴘᴘᴏʀᴛ ⚜️', callback_data='channels')
        ],[
...
async def auto_filter(client, msg, s, spoll=False):
    ...
    if offset != "":
        if settings['shortlink']:
            btn.insert(0,
                [InlineKeyboardButton("Lᴀɴɢᴜᴀɢᴇs", callback_data=f"languages#{key}#{req}#{offset}"),
                InlineKeyboardButton("Qᴜᴀʟɪᴛʏ", callback_data=f"quality#{key}#{req}#{offset}")]
            )
            btn.insert(1,
                [InlineKeyboardButton("📥 𝗦𝗲𝗻𝗱 𝗔𝗹𝗹 𝗙𝗶𝗹𝗲𝘀 📤", url=await get_shortlink(settings['url'], settings['api'], f'https://t.me/{temp.U_NAME}?start=all_{message.chat.id}_{key}'))]
            )
        else:
            btn.insert(0,
                [InlineKeyboardButton("Lᴀɴɢᴜᴀɢᴇs", callback_data=f"languages#{key}#{req}#{offset}"),
